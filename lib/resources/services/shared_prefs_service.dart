...
class SharedPrefsService {
  ...
  Future<void> saveUserModel(UserModel value) async {
    ...
    final String encryptedUserData = await EncryptionService().encrypt(
      userData,
    );
  ...
  Future<UserModel?> getUserModel() async {
    final String? encryptedUserData = sharedPreferences.getString(
      DotenvManager.userModelPrefsKey,
    );
    if (encryptedUserData == null) return null;
    final String userData = await EncryptionService().decrypt(
      encryptedUserData,
    );
